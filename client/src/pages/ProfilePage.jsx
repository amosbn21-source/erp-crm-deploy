...
function PlatformDisplay({ account }) {
  const getPlatformConfig = (platform) => {
    const configs = {
      ...
      'twilio': {
        icon: <SmsIcon sx={{ color: '#F22F46' }} />,
        name: 'Twilio',
        color: '#F22F46'
      },
      'sms': {
        icon: <SmsIcon sx={{ color: '#2196F3' }} />,
        name: 'SMS',
        color: '#2196F3'
      },
      'telegram': {
    ...
    return configs[account.platform] || {
      icon: <KeyIcon />,
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
      ...
      <Box>
        ...
        <Typography variant="caption" color="text.secondary">
          {account.platform_type === 'facebook_messenger' ? 'Page' : 'Compte'}
        </Typography>
      </Box>
    </Box>
  );
...
function TelegramIcon({ color = 'primary' }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={color}>
      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.57-1.38-.93-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.06-.2-.07-.06-.17-.04-.24-.02-.1.02-1.79 1.14-5.06 3.35-.48.33-.91.49-1.3.48-.43-.01-1.27-.24-1.89-.44-.76-.24-1.36-.37-1.31-.78.03-.24.36-.48 1-.73z" />
    </svg>
  );
}